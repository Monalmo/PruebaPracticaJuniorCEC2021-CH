<template>
  <div class="contenedor-flex">
    <div
      class="contenedor-calendario"
      :style="{ border: '1px solid #d9d9d9', borderRadius: '4px' }"
    >
      <div class="instrucciones">
        <h3>
          Selecciona una fecha para ver la imagen de la tierra en ese momento
        </h3>
      </div>
      <a-calendar
        :fullscreen="false"
...
export default {
	...
	methods: {
		...
		async ondia (dia) {
			...
			try {
				const imgDisponibles = await axios.get(
					'https://api.nasa.gov/EPIC/api/natural/date/' + fechaaxios + key
				)
				// eslint-disable-next-line no-console
